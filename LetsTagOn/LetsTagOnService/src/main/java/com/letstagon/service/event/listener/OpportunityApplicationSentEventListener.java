 ...
 * component's <code>addOpportunityApplicationSentEventListener<code> method. When
 * the opportunityApplicationSentEvent event occurs, that object's appropriate
 * method is invoked.
 *
 * @see OpportunityApplicationSentEventEvent
 */
@Component
public class OpportunityApplicationSentEventListener extends BaseNotificationEventListner implements ApplicationListener<OpportunityApplicationSentEvent>{
	...
	public void onApplicationEvent(OpportunityApplicationSentEvent event) {
		...
		this.createNotification(notification);
			...
			LOG.info("Opp application sent status Notification has not been sent check privacy settings to user id"+event.getParty().getUserBean().getId());
		}
		if(sendEmailAlerts){
			this.sendEmail(event);
		}else {
			LOG.info("Opp application sent status Email has not been sent check privacy settings to user id"+event.getParty().getUserBean().getId());
		}
		...
		MimeMessagePreparator preparator = new MimeMessagePreparator() {
			...
			public void prepare(MimeMessage mimeMessage) throws Exception {
				...
				model.put("regMessage",
						CommonConstants.APPLICATION_SENT_EVENT_MESSAGE);
				model.put("name", event.getParty().getUserBean().getName());
				String text = VelocityEngineUtils.mergeTemplateIntoString(
						velocityEngine, "velocity/oppApplicationSentTemplate.vm", "UTF-8",
						model);
