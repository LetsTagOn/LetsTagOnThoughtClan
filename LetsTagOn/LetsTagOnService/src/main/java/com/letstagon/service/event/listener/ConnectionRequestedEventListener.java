...
/**
 * The listener interface for receiving connectionRequestedEvent events.
 * The class that is interested in processing a connectionRequestedEvent
 * event implements this interface, and the object created
 * with that class is registered with a component using the
 * component's <code>addConnectionRequestedEventListener<code> method. When
 * the connectionRequestedEvent event occurs, that object's appropriate
 * method is invoked.
 *
 * @see ConnectionRequestedEventEvent
 */
@Component
public class ConnectionRequestedEventListener extends BaseNotificationEventListner implements ApplicationListener<ConnectionRequestedEvent> {
	...
	public void onApplicationEvent(ConnectionRequestedEvent event) {
		...
		if(sendNotification){
			...
			notification.setContent(NotificationTypeEnum.CONNECTION_REQUEST.getMessage()+" by "+event.getFromParty().getUserBean().getName());
			...
			try {
				jsonObject.put("senderId", event.getFromParty().getId());
				jsonObject.put("senderUserId",event.getFromParty().getUserBean().getId());
				jsonObject.put("senderName", event.getFromParty().getUserBean().getName());
				jsonObject.put("senderProfilePicture", event.getFromParty().getUserBean().getProfilePicture());
			...
			LOG.info("Connection request Notification has not been sent check privacy settings to user id"+event.getFromParty().getUserBean().getId());
		}
		if(sendEmailAlerts){
			this.sendEmail(event);
		}else {
			LOG.info("Connection request Email has not been sent check privacy settings to user id"+event.getFromParty().getUserBean().getId());
		}
		...
		MimeMessagePreparator preparator = new MimeMessagePreparator() {
			...
			public void prepare(MimeMessage mimeMessage) throws Exception {
				...
				model.put("regMessage",
						event.getFromParty().getUserBean().getName()+CommonConstants.CONNECTION_REQUEST_EVENT_MESSAGE);
				model.put("name", event.getToParty().getUserBean().getName());
				String text = VelocityEngineUtils.mergeTemplateIntoString(
						velocityEngine, "velocity/connectionRequestTemplate.vm", "UTF-8",
						model);
