var opportunityManagement = angular.module('opportunityManagementModule', ['angularjs-dropdown-multiselect']);

opportunity.controller('ManageOppApplicationsController', function($http, $scope, $rootScope, $location, $timeout) {
    // $scope.gPlace;
    var url = window.location.href
        .split("/opportunity/manageApplications");
    var oppID = url[1];
    $scope.oppID = oppID;
    $http({
    ...
    function getResultsPage(pageNumber) {
        ...
        // be in a service
        $http.get('/opportunity/' + $scope.oppID + '/participantsByStatus?status=&size=10&page=' + (pageNumber - 1))
...
opportunity
    .controller(
        'ManageOppAttendanceController',
        function($http, $scope, $rootScope, $location, $timeout) {
            // $scope.gPlace;
            var url = window.location.href
                .split("/opportunity/manageAttendance");
            var oppID = url[1];
            $scope.oppID = oppID;
            $http({
                    url: '/opportunity/' + oppID,
            ...
            $scope.manageAttendence = function(participationID, attended) {
                $('#loading-indicator').show();
                $http({
                    url: '/opportunity/participation/' + participationID + '/attendance',
                    dataType: 'json',
                    method: 'POST',
                    params: {
                        "attended": attended
...
opportunity
    .controller(
        'ManageOppFeedbackController',
        function($http, $scope, $rootScope, $location, $timeout) {
            // $scope.gPlace;
            var url = window.location.href
                .split("/opportunity/manageFeedback");
            var oppID = url[1];
            $scope.oppID = oppID;
            $http({
                    url: '/opportunity/' + oppID,
            ...
            $scope.submitFeedBack = function(participationID) {
                ...
                $scope.participation.review = $("#feedback" + participationID).val();
                $http({
