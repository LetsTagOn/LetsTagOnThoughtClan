...
searchModule.controller('SearchController',
    function($http, $scope, $rootScope, $location, $timeout) {
        ...
        $("#opportunityStartDate").datepicker({
            format: "yyyy/mm/dd",
            todayHighlight: true,
        ...
        $scope.getMasterData = function() {
            ...
            $http({
                ...
                function(error) {
                    console.log("error while getting search details of user with id:" +
                        $rootScope.userId);
                });
        ...
        $scope.getSearchKeyWordFromUrl = function() {
            ...
            if (window.location.href.indexOf("/search/user/") > -1) {
                ...
                keyword = url[1];
            } else if (window.location.href.indexOf("/search/opportunity/") > -1) {
            ...
            if (causeCounts) {
                ...
                for (var j = 0; j < causeCounts.length; j = j + 2) {
                    ...
                    for (i in $scope.masterCauseList) {
                        var cause = $scope.masterCauseList[i];
                        // cause.count=0;
                        if (cause.name.toLowerCase() === causeCounts[j]
                            .toLowerCase()) {
            ...
            if (locationCounts) {
                for (var j = 0; j < locationCounts.length; j = j + 2) {
                    ...
                    $scope.selectedLocation.forEach(function(selectedLocation) {
                        if (location.name == selectedLocation.name && location.count == selectedLocation.count) {
            ...
            if (jobTypeCounts) {
                ...
                for (var j = 0; j < jobTypeCounts.length; j = j + 2) {
                    ...
                    for (i in $scope.masterSkillList) {
                        var jobType = $scope.masterSkillList[i];
                        // jobType.count=0
                        if (jobType.name.toLowerCase() === jobTypeCounts[j]
                            .toLowerCase()) {
        ...
        $scope.getCoreName = function() {
            ...
            $scope.searchMode = $("input[name=searchMode]:checked")
                .val();
        ...
        $scope.getFilterQueryString = function() {
            ...
            if (skills.length > 0 && causes.length > 0 && locations.length > 0) {
                fq = "&fq=( " + jobTypeFq + " AND " + causesFq + "AND" + locationFq +
                    ")";
            }
        ...
        $scope.getQueryString = function() {
            ...
            if (undefined != $scope.searchString &&
                $scope.searchString != '') {
                ...
                if ($scope.searchMode == "users") {
                    ...
                    if ($rootScope.startEndDateRange) {
                        if ($("#opportunityEndDate").val() != "" && $("#opportunityStartDate").val() != "") {
                            endDate = new Date($("#opportunityEndDate").val()).toISOString();
                            startDate = new Date($("#opportunityStartDate").val()).toISOString();
                            dateRangeQuery = "&fq=dateStart:[" + startDate + " TO " + endDate + "]" + "&fq=dateEnd:[" + startDate + " TO " + endDate + "]";
                        } else if ($("#opportunityEndDate").val() == "" && $("#opportunityStartDate").val() != "") {
                            startDate = new Date($("#opportunityStartDate").val()).toISOString();
                            dateRangeQuery = "&fq=dateStart:[" + startDate + " TO *]";
                        } else if ($("#opportunityEndDate").val() != "" && $("#opportunityStartDate").val() == "") {
                            endDate = new Date($("#opportunityEndDate").val()).toISOString();
                            dateRangeQuery = "&fq=dateEnd:[* TO " + endDate + "]";
                        }
                    } else {
     ...
     // Sourabh: commented this as this is being called before the field value is passed   $scope.getResultsPage(1);
            ...
            $http({
                ...
                .success(
                    function(response) {
                        ...
                        $scope.results.forEach(function(
                            resultItem) {
            ...
            $http({
                ...
                .success(
                    function(response) {
                        ...
                        resultItemObj.pendingResponse = false;
                        resultItemObj.ifPending = false;
                        $rootScope.ltoSuccessMessage = "You are successfully connected to " +
        ...
        $('#searchModeOppsDateRange').change(
            function() {
