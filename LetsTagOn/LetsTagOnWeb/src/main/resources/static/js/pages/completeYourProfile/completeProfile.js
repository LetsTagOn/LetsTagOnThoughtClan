 ...
 * 
 */
completeProfile.controller("PersonalInformationController", function(
    $http,
    $scope,
    $rootScope,
    $location,
    ...
    $http({
        ...
        .success(function(response) {
            if (response.error == null) {
                ...
                $scope.user.dateOfBirth = $filter("date")(
                ...
                if ($scope.userTypeList.length > 0) {
                    response.data.userTypeMasterList.forEach(function(
                        masterData
                    ) {
                        $scope.userTypeList.forEach(function(userData) {
                            if (
                                masterData.name == userData.userType.name &&
                                userData.active
                            ) {
                                masterData.addClass = "selected";
                            }
                            var index = $scope.userTypeMasterList.indexOf(
                                masterData
                            );
                            if (index == -1) {
                    ...
                    $scope.userTypeMasterList =
                        response.data.userTypeMasterList;
                }
                ...
                $scope.additionalAttributeQuestionList = [];
                $scope.additionalAttributeQuestionList =
                    response.data.additionalAttribute;
                $scope.prefillAdditionalAttributeValues(
                    response.data.userAdditionalAttribute
                );
            } else {
    ...
    $scope.prefillAdditionalAttributeValues = function(userAttributeList) {
        if (userAttributeList.length > 0) {
            $scope.additionalAttributeQuestionList.forEach(function(
                additionalAttribute
            ) {
                userAttributeList.forEach(function(userAttribute) {
                    if (
                        additionalAttribute.id ==
                        userAttribute.additionalProfileAttribute.id
                    ) {
    ...
    $scope.updateUserTypeMasterList = function(response) {
        $scope.userTypeMasterList.forEach(function(list) {
            if (response.userType.name == list.name && response.active) {
                ...
                response.userType.name == list.name &&
                !response.active
            ) {
                list.addClass = "";
            }
    ...
    $scope.saveAdditionalAttributeList = function() {
        ...
        $scope.additionalAttributeQuestionList.forEach(function(attribute) {
            ...
            additionalPropertyAttribute.value = $(
        ...
        $http({
            ...
            .success(function(response) {
                ...
                if (response.error == null) {
                    ...
                    $rootScope.attributeErrorMessage =
...
completeProfile
    .controller("UploadProfilePicController", function(
        $http,
        $scope,
        $rootScope,
        $location,
        ...
        $scope.saveProfilePic = function(file) {
            ...
            $http
                .post("/userProfile/uploadPic", uploadfile, {
    ...
    .directive("imagecrop", function() {
        return {
            template:
                '<div class="modal fade image-crop-modal" id="image-crop-modal">' +
