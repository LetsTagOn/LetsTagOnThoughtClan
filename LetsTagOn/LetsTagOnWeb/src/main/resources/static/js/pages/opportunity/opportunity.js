...
opportunity.controller("UploadOportunityImageController", function($http, $scope, $rootScope, $location, $timeout) {
    ...
    $scope.saveOpportunityImage = function(file) {
        ...
        $http.post('/opportunity/uploadBannerImage/' + url[1], uploadfile, {
            ...
            $(".lto-opp-banner-img").attr('src', $rootScope.baseUrl + "/opportunity/getBannerImage?key=" + response.data);
...
opportunity.controller('OpportunityController', function($http, $scope, $rootScope, $location, $timeout, $filter) {
...
opportunity.controller('CreateOpportunityController', function($http, $scope, $rootScope, $location, $timeout) {
    $scope.gPlace;
    $scope.event = {};
    ...
    $scope.createProgram = function() {
        ...
        $http({
            ...
            if (response != undefined && response.data.id != undefined) {
                $location.path("/view/opportunity/program/edit/" + response.data.id);
            } else {
    ...
    $scope.createEvent = function() {
        ...
        $http({
            ...
            if (response.data.id != undefined)
                $location.path("/view/opportunity/edit/" + response.data.id);
            else
...
opportunity.controller('OpportunityEditController', function($http, $scope, $rootScope, $location, $timeout, $filter) {
    ...
    $('#eventEndDate').datetimepicker({}).on('changeDate', function(e) {
        $scope.event.dateStart = $(this).val();
        $(this).focus();
        $(this).datepicker('hide');
    });
    ...
    $scope.removeJobFromMasterJobType = function() {
        for (i in $scope.opportunityJobTypes) {
            for (j in $scope.masterJobTypeList) {
                if ($scope.masterJobTypeList[j].id == $scope.opportunityJobTypes[i].jobTypeBean.id) {
                    console.info("comparing : " + $scope.masterJobTypeList[j].name + " --- > " + $scope.opportunityJobTypes[i].jobTypeBean.name)
    ...
    $scope.iterateJob = function() {
        ...
        for (i in $scope.opportunityJobTypes) {
            console.info(" -- > " + $scope.opportunityJobTypes[i].numberOfPositions);
        }
    };
    ...
    $scope.saveEditEvent = function() {
        ...
        if ($("input[name='formattedAddress']").val() == "") {
            fullAddress = address.street + "," + address.city + "," + address.postalCode + "," + address.state + "," + address.country;
        ...
        setTimeout(function() {
            ...
            $http({
                ...
                $('#loading-indicator').hide();
                $scope.eventActionError = "Something went wrong!!Please refresh page";
    ...
    $scope.editProgram = function() {
        ...
        $http({
            ...
            $scope.opportunityJobTypes = response.data.opportunityDTO.opportunityJobTypes;
            $scope.removeJobFromMasterJobType();
        }).error(function(error) {
...
opportunity.controller('OpportunityProgramEditController',
    function($http, $scope, $rootScope, $location, $timeout) {
        ...
        $scope.editProgramWithCreateEvent = function() {
            ...
            $http({
                ...
                $scope.program.linkedEvents = response.data.opportunityDTO.linkedEvents;
...
opportunity.controller('ViewOpportunityController', function($http, $scope, $rootScope, $location, $timeout) {
    // $scope.gPlace;
        ...
        $http({
            ...
            function(response) {
                ...
                for (i in listedJob) {
                    ...
                    for (j in results) {
                        if (listedJob[i] == results[j].jobTypeBean.description) {
                            counter++;
                        }
                    }
            ...
            function(response) {
                ...
                if (response.data.partyParticipations) {
                    ...
                    for (var participation in participationList) {
                        var jobId = participationList[participation].jobTypeBean.id;
    ...
    $scope.applyForOpp = function(opp, jobType) {
        console.log("Opp : " + opp.id + " , job type : " +
            jobType.id);
        $http({
                url: '/opportunity/' + opp.id +
                    '/party/0/jobType/' + jobType.id,
                dataType: 'json',
                method: 'POST',
                headers: {
                    "Content-Type": "application/json"
...
opportunity.controller("InviteVolunteerController", function($http, $scope, $rootScope, $location, $timeout) {
    var url = window.location.href.split("/view/opportunity/");
    var oppID = url[1];
    $scope.oppID = oppID;
    ...
    $scope.inviteVolunteer = function(party, oppDetails) {
        $http({
            url: '/opportunity/' + oppDetails.id + '/invitedBy/' + $rootScope.userId + '/invite/' + party.id,
            dataType: 'json',
            method: 'POST',
            headers: {
                "Content-Type": "application/json"
