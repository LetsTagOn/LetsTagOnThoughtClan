 ...
 * 
 */
completeProfile.controller("InterestsController", function(
    $http,
    $scope,
    $document,
    $rootScope,
    ...
    $http({
        ...
        .success(function(response) {
            if (response.error == null) {
                ...
                if (
                    typeof response.data.locationPreference != undefined &&
                    response.data.locationPreference != null
                ) {
                    $scope.includeSurroundingAreas =
                        response.data.locationPreference.includeSurroundingAreas;
                }
        ...
        $scope.volPreferredTimeList.forEach(function(list) {
            list.status.forEach(function(presentDayStatus) {
                responseList.forEach(function(response) {
                    if (
                        presentDayStatus.startTime == response.startTime &&
                        presentDayStatus.endTime == response.endTime &&
                        presentDayStatus.day == response.day
                    ) {
                    ...
                    if (response.day == "com") {
                        if (response.commitment == 2) $scope.inperson = true;
                        else if (response.commitment == 1) $scope.remote = true;
    ...
    $scope.changeJobTypeALLStatus = function() {
        ...
        var flag = true;
        $scope.masterSkills.forEach(function(skill) {
            if (!skill.status && skill.name != "ALL") {
                flag = false;
            }
        });
    ...
    $scope.saveVolAvailability = function(availabilty) {
        ...
        $http({
            url:
                "/profile/availability/user/" +
                $scope.volAvail.user.id +
                "/add",
            dataType: "json",
            method: "POST",
            data: $scope.volAvail,
    ...
    $scope.saveLocationPrefDetails = function() {
        ...
        $scope.locationPrefDTO.includeSurroundingAreas =
            $scope.includeSurroundingAreas;
        $("#loading-indicator").show();
        $http({
            url:
                "/profile/availability/location/user/" +
                $scope.locationPrefDTO.user.id +
                "/preferences",
            dataType: "json",
            method: "POST",
    ...
    $scope.checkCom = function() {
        var checked = 0;
        if ($scope.inperson && $scope.remote) checked = 3;
        if ($scope.inperson && !$scope.remote) checked = 2;
        if (!$scope.inperson && $scope.remote) checked = 1;
        $scope.volAvail = new Object();
        var user = new Object();
        user.id = $rootScope.userId;
        $scope.volAvail.user = user;
