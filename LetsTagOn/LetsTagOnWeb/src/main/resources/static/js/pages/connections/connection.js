            ...
            $scope.showUserProfile = function(user) {
                if (user.party2.userBean.id == $rootScope.userId) {
                    $location.path("/profile/user/" +
                        user.party1.userBean.id);
                } else {
                    $location.path("/profile/user/" +
                        user.party2.userBean.id);
                }
                            ...
                            $scope.displayConnectionSuggestionList
                                .forEach(function(
                                    ...
                                    if (currentConnention.id == response.party2.id) {
                                        var index = $scope.displayConnectionSuggestionList
                                            .indexOf(currentConnention);
                                        $scope.displayConnectionSuggestionList
                                            .splice(
                                                index,
                                                1);
    ...
    .controller(
        ...
        function($http, $scope, $rootScope, $location, $timeout) {
            ...
            function getPendingConnectionsPerPage(pageNumber) {
                ...
                $http
                    .get(
                        '/connection/' + $rootScope.userId + '/openRequests?size=' +
            ...
            $scope.connectionReject = function(connection) {
                $http({
                        url: '/party/' + connection.party1.id +
                            '/connection/reject/' +
                    ...
                    .success(
                        function(response) {
                            $rootScope.pendingConnectionList
                                .forEach(function(
                                    currentConnention) {
                                    if (currentConnention.party2.id == connection.party2.id) {
                                        var index = $rootScope.pendingConnectionList
                                            .indexOf(currentConnention);
                                        $rootScope.pendingConnectionList
                                            .splice(
                                                index,
            ...
            $scope.showPendingConnections = function() {
                $location.path("/pending/connection/user/" +
                    $rootScope.userId);
            };
