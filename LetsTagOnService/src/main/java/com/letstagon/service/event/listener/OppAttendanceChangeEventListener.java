...
/**
 * The listener interface for receiving oppAttendanceChangeEvent events.
 * The class that is interested in processing a oppAttendanceChangeEvent
 * event implements this interface, and the object created
 * with that class is registered with a component using the
 * component's <code>addOppAttendanceChangeEventListener<code> method. When
 * the oppAttendanceChangeEvent event occurs, that object's appropriate
 * method is invoked.
 *
 * @see OppAttendanceChangeEventEvent
 */
@Component
public class OppAttendanceChangeEventListener extends BaseNotificationEventListner implements ApplicationListener<OppAttendanceChangeEvent>{
	
	/** The Constant LOG. */
	private static final Logger LOG = LoggerFactory.getLogger(OppAttendanceChangeEventListener.class);
	...
	@Override
	@Async
	public void onApplicationEvent(OppAttendanceChangeEvent event) {
		LOG.trace("OppAttendanceChangeEventListener occured");
		...
		notification.setContent(NotificationTypeEnum.OPPORTUNITY_ATTENDENCE_CHANGE.getMessage());
		notification.setPartyBean(event.getParty());
		notification.setType(NotificationTypeEnum.OPPORTUNITY_ATTENDENCE_CHANGE.getEventType());
		...
		this.createNotification(notification);	
			...
			LOG.info("Opp attendance status Notification has not been sent check privacy settings to user id"+event.getParty().getUserBean().getId());
		}
		if(sendEmailAlerts){
			this.sendEmail(event);
		}else {
			LOG.info("Opp attendance status Email has not been sent check privacy settings to user id"+event.getParty().getUserBean().getId());
		}
	}
	 ...
	 */
	public void sendEmail(OppAttendanceChangeEvent event){
		MimeMessagePreparator preparator = new MimeMessagePreparator() {
			...
			public void prepare(MimeMessage mimeMessage) throws Exception {
				...
				model.put("regMessage",
						CommonConstants.ATTENDANCE_CHANGE_EVENT_MESSAGE);
				model.put("name", event.getParty().getUserBean().getName());
				String text = VelocityEngineUtils.mergeTemplateIntoString(
						velocityEngine, "velocity/oppAttendanceTemplate.vm", "UTF-8",
						model);
